...
class UserService extends BaseService
{
    ...
    public function userUpdate($post)
    {
        ...
        if ($is_save) {
            LogService::service()->saveLog($_COOKIE['account'], Log::MODULE_USER, $_COOKIE['account'] . '编辑了用户信息' . $model->account, json_encode($diff));
    ...
    public function changePwd($post)
    {
        ...
        $is_save = $model->save();
        if ($is_save) {
            LogService::service()->saveLog($_COOKIE['account'], Log::MODULE_USER, $_COOKIE['account'] . '修改了密码' . $model->account);
        }
        return $is_save;
    }
